...
**CONDITIONAL** — with the same stated deviation as the sibling review.

By the letter of the rule this is a FAIL: two tasks are incomplete. Both are
parked on the FR-003-CON-1 sign-off, which is a decision, not missing work.
...
| --- | --- | --- | --- |
| FND-001 | medium | The FR-003 suite does not run in CI (`ci.yml` is `workflow_dispatch` only and installs no `quire`), and the module-level `skipif` makes a missing CLI a silent skip rather than a failure — a green run that verified nothing. Locally it runs against whatever `quire` is on `PATH`; because the manifest's top-level struct does not deny unknown fields, an older CLI silently ignores the `traceability:` block while the drift guard still passes. | FR-003, tests/test_testmatrix_body_extraction.py |
