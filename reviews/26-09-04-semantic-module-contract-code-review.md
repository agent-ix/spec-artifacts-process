...
| FND-004 | medium | **Superseded by PLAT-902 (2026-09-20): both copies and the split check are deleted; this records what was true on 2026-09-04 and is not a standing design.** The FR-035 conformance check is split in two — the manifest minus `semantic` against the shipped schema, and the block against an extracted sub-schema — because no published revision knows both `traceability`/`verification_catalog` and `semantic`. Both halves are checked; neither is dropped. `agent-ix/filament-core-service#27`. | tests/test_manifest.py:113 |
