...
- Each `Traces To` cell **SHALL** be constrained via the Quire
  `column_patterns` assert to one or more comma-separated requirement/AC id
  tokens matching
  `^(StR|US|FR|NFR)-\d+(-AC-\d+)?(,\s*(StR|US|FR|NFR)-\d+(-AC-\d+)?)*$`;
...
| FR-003-AC-6 | A `Traces To` cell with no requirement/AC id token fails via `column_patterns` | Test (TC-001) |
