...
| FR-007-AC-8 | The traceability model declares the ecosystem's obligation sources — the three acceptance-criterion targets (`FR`, `NFR`, `interface`; quire-rs#460 added the third), the NFR measurement table, and the configuration-dimensions table — so quire-rs FR-053 derives obligations rather than shipping inert. | Test (TC-055) |
