...
| FR-004 | FR-004-AC-11 | TC-071 | ✅ Complete |
| FR-004 | FR-004-AC-12 | TC-072 | ✅ Complete |
...
| FR-004 | FR-004-AC-18 | TC-079 | ✅ Complete |
...
| TC-078 | No trace target mints from a section no archetype's `body_extraction` declares — no `Invariants` entry in any spelling, no `NFR`-bound `Constraints` entry. The declared FR `Constraints` table is minted by an explicitly optional target, its `Validation` references are read, and the manifest records the decisions for `SC`, `CON`, and bare `FR-nnn` with their owning tickets (#244, #327, #328; CR-043). | Unit | P0 | FR-004-AC-17 | ✅ |
| TC-079 | `test-case` and `traces-to` use the same section family — `*Test Case Summary*` plus `Integration Test Matrix` — while the noncanonical `Test Cases` heading remains excluded (CR-044). | Unit | P0 | FR-004-AC-18 | ✅ |
...
| TC-043 | `suite` and `inspection` are archetype-bound reference-only trace targets and `inspection-obligation` is a declared reference, so both id classes resolve without entering source-evidence coverage and a bad obligation cell dangles rather than recording an act against nothing (FR-006) | Unit | P0 | FR-006-AC-4 | ✅ |
