...
| FR-007 | FR-007-AC-15 | TC-144 | ✅ Complete |
...
| TC-048 | The catalog covers every technique the ADR-0011 survey dispositioned plus shared additions — 34 methods across all four IADT classes, not a starter subset: an absent entry reads as "not applicable" rather than "not yet written" (FR-007) | Unit | P0 | FR-007-AC-1 | ✅ |
...
| EC-005 | A `Traces To` cell using both shorthands at once (`FR-016-AC-1/2, -AC-5`) | FR-011-AC-6 | TC-102 | Two authored abbreviations expand differently in different readers |
